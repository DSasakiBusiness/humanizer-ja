...
v0.4.0 / v0.3.1 の検証結果を [EVALUATION.md](./EVALUATION.md) に記載しています（v0.1.0 〜 v0.3.0 の検証履歴は git コミット履歴から参照可能）。

### v0.4.0：日本語人間コーパス分析（17,367 件、約 1,150 万文字）

合成サンプルではなく **実際の日本語コーパス** を解析して各パターンに「人間ベースライン記事率」を付与。検出強度を 4 段階に分類した。

| コーパス | 記事数 | register |
|---|--:|---|
| livedoor news | 7,367 | 常体寄りの news 記事 |
| CC-100 ja | 10,000 | 敬体寄りの Web 多様（個人ブログ・企業サイト等） |

主要発見：

- **超強検出（人間 < 0.1%）9 パターン**：枕詞、太字濫用、インラインヘッダ箇条書き、絵文字、細切れ見出し、チャットボット痕跡、知識カットオフ言い訳、媒体露出インフレ、課題と展望型 → **単発でも AI 臭の確度が極めて高い**
- **連発検出（人間 ≥ 3%）6 パターン**：括弧多用、機械的接続詞、〜することができる、形容過多、断定回避、三点列挙 → **人間も普通に使う、連発時のみ叩く**
- パターン 33（括弧）の閾値を実データで再校正（livedoor の 80.97% を踏まえ news 系で大幅緩和）

再現可能：`tools/analyze.py` + `tools/fetch_cc100.py` でいつでも再測定可能。データは `tools/results/` に commit 済み。

### v0.3.0 / v0.3.1 検証のハイライト

- **5 モード機能性**：5 モードすべてで期待通りの書き分け
- **AskUserQuestion ガードレール**：6/6 で期待通り発火／非発火
- **rewriter 自己 AI 臭再導入チェック**：4/4 検出
- **5 モード自動推定**：8/8 正解
- **technical-neutral サブ規約**：API リファレンス／チュートリアル／解説記事

### 評価方法の限界

- 単一モデル自己評価（書き直し・採点ともに Claude が実施、独立性なし）
- 実機 `AskUserQuestion` 動作の確認は別環境必要
- 盲検人間評価は本リポジトリ内では実施不可、EVALUATION.md に**ユーザー側で実施するための推奨手順**を記載
- AI 出力との直接比較は v0.5.0 以降で実施予定（人間ベースラインのみ確立済み）

### 実利用前の推奨

1. ローカル Claude Code で humanizer-ja をロードし AskUserQuestion 動作を確認
2. EVALUATION.md の盲検評価手順を 3 名以上で実施
